...
var treeToHTML = (node, level = 0, minimize = false) => {
  let result = "";
  if (level === 0) {
    result += `<!DOCTYPE html>
<html>
  ...
  const lastTag = "div";
  result += `    ${indent}<${lastTag} class="block-element ${
    node.clickable ? "clickable-element" : ""
  }">\n`;
  result += `    ${indent}  <p>ID=${
    node.id
  } <span class="attrs">${node.tagName.toLowerCase()}`;
  ...
  result += `</span></p>\n`;
  if (node.content) {
    result += `    ${indent}  <p>${node.content}</p>\n`;
  }
  ...
  if (node.children) {
    ...
    if (node.children.length <= maxChildren) {
      for (const child of node.children) {
        result += treeToHTML(child, level + 1, minimize);
      }
    } else {
      for (const child of node.children.slice(0, maxChildren / 2)) {
        result += treeToHTML(child, level + 1, minimize);
